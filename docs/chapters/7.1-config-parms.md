...
#### Stationary Zone - A temporary zone

A Stationary Zone is a zone that is created when the device has been in the same location for a period of time (at a mall, doctor's office, restaurant, friend's house, etc). The purpose of this zone is to reduce location requests when the device has not moved. It is a temporary zone that is removed when all of the devices have exited from it. A custom name can not be assigned to them other the friendly name that is configured on this screen.

![](../images/cf-special-zones-stationary.png)

##### Home Assistant Zones - A permanent zone

A Stationary Zone is not useful for zones where a device will be in the same location for a long period of time on a regular basis (work, gym, store regularly visited, friends house, another house, parents, etc. )
