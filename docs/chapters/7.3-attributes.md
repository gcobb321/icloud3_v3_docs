...
| Interval Sensor                       | Distance Moved Sensor                     |
| ---------------------------------- | ------------------------------------ |
| ![](../images/attrs-interval.png) | ![](../images/attrs-distance-moved.png) |
...
- as a string value - The attribute displayed on *sensor.gary_iphone_home_distance* for '--Lillian/iPhone' is 6.00mi or 9.64km. These attribute names start with '--',
- as a numeric value - The actual value is stored internally as 5.98946 or 9.63912 and can be displayed on the HA Developer tools > States screen. These attribute names start with '='

The *entities* card is created with the *attributes* secondary information parameters. This is described in the HA Entities Dashboard Documentation [here](https://www.home-assistant.io/dashboards/entities/). This must be created using YAML code in the Dashboard edit screen. The following dashboard screenwas created with the yaml code below.
  ...
  - type: entities
    entities:
      ...
      - type: attribute
        ...
        name: Gary <--> Lillian Watch Distance
      - type: attribute
        entity: sensor.lillian_iphone_home_distance
        attribute: '--Gary-iPad'
        name: Lillian <--> Gary-iPad Distance
...
The yaml code for the above is:
 ...
 - platform: template
   sensors:
     ...
     distance_gary_to_lillian_watch:
       friendly_name: "Gary <--> Lillian Watch Distance"
       value_template: "{{ state_attr('sensor.gary_iphone_home_distance', '--Lillian-Watch') }}"
       
     distance_lillian_to_gary_ipad:
       friendly_name: "Lillian <--> Gary-iPad Distance"
       value_template: "{{ state_attr('sensor.lillian_iphone_home_distance', '--Gary-iPad') }}"
 ```
