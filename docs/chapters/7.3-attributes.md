...
!><ul>The distances between devices may be different than the *_home_distance* or *_zone_distance* state value.
!><li>The distance between devices is the *calculated distance*, the straight line distance between two gps points.</li> 
!><li>The *_home_distance* and *_zone_distance* state value may be the *Waze Route Distance*, the driving distance between the same two points that was returned from the Waze Route server.</li></ul>
