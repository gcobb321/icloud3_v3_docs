...
- **Easy Way** -  **Add iCloud3 to HACS as a custom repository**
...
Open the *Configure Settings* screens. Use Option 1 below since the Event Log has not been set up yet.

1. Select **☰ > HA  Settings > Devices & Services > Integrations** if the Integrations screen is not already displayed.

   or

   Select the **Configure iCloud3 Settings** icon on the **Event Log**.
...
3. Copy all of the code in the above code block and past it into the Edit Configuration window. 
   ...
   - Copy the code below into the blank editor area. This code is for the *gary_iphone* device_tracker entity on the left-side of the screen above.. Change all occurrences of *gary_iphone* to the iCloud3 device_tracker entity name for your phone you entered in the *Add Devices*  screen in step 3.3 above.
...
-  When iCloud3 starts, it does the following:
  ...
  - Determine if the Event Log in the */www* directory is the latest version. If not, it is updated. A browser refresh will be needed it it was updated. A notification is displayed on each device that a refresh is needed. 
...
iCloud3 writes logging records to it's */config/icloud3-debug.log* file. This includes:
...
- The debug-log file is recreated when iCloud3 is restarted. The previous file is renamed *icloud3-debug-1.log*.
- The debug log file is closed at midnight and renamed *icloud3-debug-1.log*. A new file is created for the new day.
- The log_level is changed on the *Configure Settings > Menu Page 2 > Format Settings* screen.
- Debug logging can be started and stopped temporarily on the *Event Log > Actions* drop down items list.
