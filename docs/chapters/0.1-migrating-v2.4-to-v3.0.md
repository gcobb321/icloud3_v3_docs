...
1. Install iCloud3 from HACS (or manually).  iCloud3 is available on HACS as a 
...
- ### Easy Way -  Use HACS
  ...
  2. Select **iCloud3 Device Tracker** (*iCloud3 v2 is an advanced iDevice tracker ...*). The Description screen is displayed.
  3. Click the three-dots in the upper-right corner (**⋮**), then select **Redownload** or **Download**.
  4. **Enable Show beta versions**.
  5. Select **v3.0-pr1#** in the dropdown list. 
     This assumes you have configured HACS to use the default *Number of releases to show* value (5). If not, go to Devices & Services > HACS > Configure. Set the value to 5 and select Submit.
  6. Select **Download**.
