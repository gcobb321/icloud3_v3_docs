...
The Dashboard Builder is a tool for creating and updating iCloud3 that show tracking results and other device information. The dashboards contain items that can be copied and pasted onto other dashboards in several formats:
...
Six views tabs are set up when a dashboard is created.
...
- ***Tracking Results** - The Tracking info consists of three sensor glance cards - Tracking Results, Tracking status, Device Info Msg. This screen shows the Tracking Results Summary for all devices
...
A dashboard is created on the *iCloud3 Dashboard Builder* screen. 

1. Select **Create a new iCloud3 Dashboard**
