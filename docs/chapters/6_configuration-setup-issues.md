...
+ Why is the HACS version number not the same as the iCloud3 that is running  +

  HACS keeps that version number in its database to be able to identify when an update is available.

	![](../screens/home_assistant/version-hacs.png)
...
+ I'm getting a *Custom element doesn't exist: icloud3-event-log-card* error setting up the Event Log Card +
  ...
  - **Check the Directory name in the iCloud3 Configuration** - Verify the directory name containing the Event Log's *icloud3-event-log-card.js* file is correct on the *Configure Settings > Menu #2 > Display Settings & Other Parameters* screen. It is listed at the bottom of the parameter list. If not, select the correct directory in the list.
  ...
  - **Find and verify that the Event Log Custom Card is in the list** - Verify that */local/icloud3/icloud3-event-log-card.js* is in the list of resources. Add it if it is not. See the *Core Component Details > Event Log* item  <a href="#/chapters/2_event-log?id=lovelace-resource-configuration">here</a> for more information. Be sure the value you enter for the resource starts with */local*. It does not start with */www*.


  - **Find and verify the iCloud3 entry** - The item  */local/icloud3/icloud3-event-log-card.js* should be in the Resources list. Add it if it is not. The value you enter for the resource starts with */local*. It does not start with */www*.


  - **Check the value you are entering when setting up the Dashboard** - The value you specify for the *icloud3-event-log-card* custom card when you add it to a Dashboard screen is `type: 'custom:icloud3-event-log-card'`.  More information about this is in the *iCloud3 Components, Event Log* documentation <a href="#/chapters/2_event-log?id=lovelace-resource-configuration">here</a>.
