...
- The *Update Apple Account* screen is displayed. You enter your username/email-id and password, iCloud3 logs into your account.
- Apple will display a pop-up notification window letting you know that someone is signing into your account. It will want the 6-digit verification code or a security key keypress to approve the sign-in. The *Authenticate Apple Sign-in* screen is displayed. Follow the instructions to enter the code or press the security key.
- The devices in your Apple account were downloaded when you signed in. They are analyzed and iCloud3 device items were created for you to import. The *Import Apple Devices* screen is displayed with these device items categorized into tracked devices (iPhone, Watch), monitored devices (iPad, Mac) and inactive devices (all others). Import the tracked and monitored devices and the iCloud3 device_tracker and sensor entities are created.  Note: Tracked devices are actively located, Monitored devices use data downloaded with tracked devices.
- After importing, the *iCloud3 Devices* screen is displayed. You can edit the created devices (Name, Picture, Mobile App assignment, etc) if necessagy.
- End the *iCloud3 Configure Session* (select Menu, then Exit).
- The *iCloud3 Dashboard* is created and added to the *HA Sidebar*. Click the Cloud icon (<img src="../images2/cloud-icon-42x42.png" width="20" height="16">) to display your devices and the *Event Log*.
...
![](../screens/apple_acct/pple-acct-update-first-time.png)
...
- Select **Menu**, then click **Submit** to display to the *Configure Devices and Sensors* menu.
