...
7. iCloud3 can not find the configured the Apple device in the data Apple sent back. Check the *Configure > Update iCloud3 Device* screen and check the Apple iCloud3 Device selection field.
...
However, when you are using several Apple Accounts and both apple accounts are sharing the same devices, you can limit the devices located during a location request to those 'owned' by that Apple Account. There are benefits and detrements associated with each method.
...
Several events take place when you upgrade a device, i.e, replace an iPhone 17 with an iPhone 18.
...
- Rename the iPhone 18 Pro Max from *Gary iPhone 17 (2)* back to *Gary iPhone* after you transfer the apps, data and settings.
- Restart iCloud3 to download the updated devices from Apple with the new internal *device_id*.

Since the new iPhone 18 Pro Max is now the *Gary iPhone* device, it is now be assigned to the same iCloud3 Device.
...
- When it is back up, www.icloud.com is checked to make sure a location a request is successful
