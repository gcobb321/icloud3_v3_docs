...
+ What is the difference between *Locate Device(s) using Apple Accounts*  vs *Send Locate Request to Mobile App* +
  ...
  - **Apple Account Locate** - (Preferred) iCloud3 requests the location from iCloud Location Services and gets an immediate response with its location. It also gets the location of the other devices in the Family Sharing list.
  - **Mobile App Request** - iCloud3 sends a message to that iDevice asking for its location and then waits for a response. There may be a delay in providing the location if the device is asleep, the Mobile App is not loaded and running or if it is running in the background.
...
+ How can I display a map of where my iPhone has been +

  HA provides a Lovelace map card that will show the location track of your devices. Refer to the HA Map documentation on setting it up. In the entity field, add each Person (_person.gary_) or device (<em>device_tracker.gary_iphone</em>) that you want to track on the Map configuration screen.
...
+ Occasionally, my iPhone does not Exit the Home Zone when I leave +
  ...
  You will never know why events work fine most of the time but fail occasionally. It could be the Mobile App on your iPhone was asleep and did not wake up to issue an Exit Trigger or it was not running, the cell signal was poor at the time, the location was old or the gps accuracy was poor, the device without the Mobile App left the zone and then returned before the next update time was reached, etc.
...
+ My iPhone never exits the Home Zone or mine exits but my wife's does not +
  ...
  - Go to *Configure > iCloud3 Devices > Update iCloud3 Device* screen, review  the *Mobile App Device* field and update it if necessary.

  - The HA mobile app integration is not installed.

  - Go to the *Using the HA Mobile App > Configure the Home Assistant Companion App* chapter for more information.
...
+ The Mobile app device_tracker entity location is not being updated by the Mobile app +
  ...
  - Verify the Mobile App is sharing location information. Go to the *Using the HA Mobile App > Configure the Home Assistant Companion App* chapter for screenshots of the correct settings.
