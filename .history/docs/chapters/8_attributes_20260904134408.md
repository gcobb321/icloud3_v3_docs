...
!> NOTE: The distances between devices may be different than the *_home_distance* or *_zone_distance* state value when one device is at Home and the other is not. Different methods are used to determine the distance for the sensor's State Values and the value in the Sensor's attributes.
  - Sensor's Attribute Device *_distance* value - This is the calculated straight line distance between the location of both devices.
  - State value for the *_home_distance* and *_zone_distance* sensor entity - These are the driving distance from the device's last location and the Home zone or another tracked-from-zone which was returned from the Waze Route Service on the last tracking update.
